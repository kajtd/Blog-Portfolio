<template>
  <NuxtLink :to="article._path" class="group">
    <article
      class="h-full max-w-sm p-6 space-y-6 rounded-2xl shadow-xs border border-gray-300/70 bg-white min-h-52 w-full flex flex-col justify-between"
    >
      <img
      ...
      <h2
        class="text-base font-semibold font-display tracking-tight text-gray-800 group-hover:text-primary-900"
      >
        {{ article.title }}
      </h2>
      ...
      <time
        ...
        class="relative mb-3 flex items-center text-sm text-gray-400 dark:text-gray-500"
        datetime="2022-09-05"
      >
